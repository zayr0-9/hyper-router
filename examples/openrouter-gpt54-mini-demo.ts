...
const agent = defineAgent({
  name: "openrouter-gpt54-mini-demo-agent",
  instructions:
...
async function main(): Promise<void> {
  ...
  console.log("This demo makes real OpenRouter API calls and persists transcript state to JSON.\n");

  const runtimeA = await createDemoRuntime();
  const firstRun = await runtimeA.run({
    sessionId,
    input: "Call the echo tool with text='hello from GPT-5.4 Mini demo' and then briefly explain what happened.",
    maxSteps: 4,
  });
  ...
  const storage = new JsonStorage({ filePath: storagePath });
